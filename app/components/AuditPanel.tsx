...
type Audit = NonNullable<RunResult["qa"]>["audit"];
    ...
    <div>
          ...
          <div style={S.questionList}>
            {audit?.discovery_questions?.map((q, i) => (
              <div key={i} style={S.questionCard(q.priority)}>
                <div style={S.questionPriority(q.priority)}>
      ...
      <div style={S.scopeCard}>
        <div style={S.scopeRow}>
          ...
          <span
            style={S.scopeBadge(
              audit?.scope_reality_check?.budget_vs_scope ?? "",
            )}
          >
            {audit.scope_reality_check?.budget_vs_scope.toUpperCase()}
          </span>
        </div>
        <div style={S.scopeAssessment}>
          {audit.scope_reality_check?.assessment}
        </div>
        <div style={{ marginTop: "8px" }}>
          <ConfidenceBar value={audit.scope_reality_check?.confidence ?? 0} />
        </div>
      </div>
