...
export function FieldPanel({ field }: { field: Field }) {
  ...
  const other    = field.tensions.filter(t => t.state !== "resolved" && t.state !== "partial").length;
    ...
    <div>
      ...
      <div style={S.confidenceCard}>
        <div style={S.confidenceRow}>
          ...
          <span style={S.confidenceLabel}>{resolved}✓ {partial}◑ {other}○</span>
        </div>
        <ConfidenceBar value={field.globalConfidence} />
        <div style={S.summary}>{field.summary}</div>
      </div>

      <div style={S.tensionList}>
        {field.tensions.map(t => <TensionCard key={t.id} t={t} />)}
      </div>
    </div>
  );
}
