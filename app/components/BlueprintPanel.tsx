...
type Blueprint = RunResult["architect"]["blueprint"];
          ...
          <div style={S.componentGrid}>
            {blueprint.components.map((c) => (
              <div key={c.name} style={S.componentCard(c.type)}>
                <div style={S.componentType(c.type)}>{c.type.toUpperCase()}</div>
            ...
            {blueprint.data_model.map((e) => (
              <div key={e.entity} style={S.entityCard}>
                ...
                {e.relations.length > 0 && (
                  <div style={{ ...S.entityMeta, marginTop: "3px", color: "#6c63ff" }}>
                    {e.relations.join(" · ")}
                  </div>
                )}
                {e.notes && (
                  <div style={{ ...S.entityMeta, marginTop: "4px", fontStyle: "italic" }}>
                    {e.notes}
                  </div>
                )}
