...
export class OntoEngine {
  ...
  equilibrate(field: Field): Equilibrium {
    ...
    while (pass < this.maxPasses) {
      ...
      for (const [id, tension] of field.tensions) {
        ...
        // Attempt resolution
        const result = this.attemptResolution(
          tension,
          field.tensions,
          pass,
          globalTrace,
  ...
  private activate(
    ...
    if (tension.state.phase === "equilibrated") return;
        ...
        for (const [kid, knowledge] of linked.knows) {
          ...
          if (!existing || knowledge.confidence > existing.confidence) {
            ...
            tension.knows.set(kid, {
              ...knowledge,
              derivedFrom: [
                ...(knowledge.derivedFrom ?? []),
                linked.id as unknown as KnowledgeId,
              ],
            });
          }
        }
  ...
  private attemptResolution(
    ...
    for (const path of tension.resolves) {
      ...
      const blockingDoubt = tension.doubts.find(
        (d) =>
          d.severity === "blocking" &&
    ...
    const successPaths = activePaths.filter(
      (p) => p.kind === "success",
    ) as SuccessPath[];
    const partialPaths = activePaths.filter(
      (p) => p.kind === "partial",
    ) as PartialPath[];
    ...
    const resistancePaths = activePaths.filter(
      (p) => p.kind === "resistance",
    ) as ResistancePath[];
    ...
    return {
      kind: "resisting",
      reason: "All paths exhausted without resolution",
    };
  }
    ...
    for (const linkedId of tension.linkedTo) {
      ...
      for (const [kid, localK] of tension.knows) {
        ...
        if (JSON.stringify(localK.value) !== JSON.stringify(linkedK.value)) {
          const localIsNewer =
            localK.origin.timestamp > linkedK.origin.timestamp;
  ...
  private isKnowledgeValid(k: SituatedKnowledge): boolean {
    ...
    if (k.validUntil.type === "until")
      return Date.now() < k.validUntil.timestamp;
  ...
  private pathMatchesId(path: ResolutionPath, id: string): boolean {
    if (path.kind === "success")
      return path.outcome === id || String(path.outcome).includes(id);
    return false;
  }
...
export function renderEquilibrium(eq: Equilibrium): string {
  ...
  if (eq.partial.size > 0) {
    ...
    for (const [id, path] of eq.partial) {
      lines.push(
        `    [${id}] — missing: ${(path.missing ?? []).join(", ") || "unspecified"}`,
      );
    }
    lines.push("");
  }
  ...
  for (const step of eq.trace) {
    lines.push(
      `    [pass ${step.pass}] ${step.action}${step.note ? " — " + step.note : ""}`,
    );
  }
